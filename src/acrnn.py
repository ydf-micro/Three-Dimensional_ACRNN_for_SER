...
def batch_norm_wrapper(inputs, is_training, decay=0.999):
    ...
    pop_mean = tf.Variable(tf.zeros([inputs.get_shape()[-1]], dtype=tf.float64), trainable=False, dtype=tf.float32)
    pop_var = tf.Variable(tf.ones([inputs.get_shape()[-1]], dtype=tf.float32), trainable=False, dtype=tf.float32)

    if is_training is not None:
        batch_mean, batch_var = tf.nn.moments(inputs, [0])  # return mean, variance
        train_mean = tf.assign(pop_mean, pop_mean * decay + batch_var * (1 - decay))
...
def acrnn(inputs, num_classes=4,
    ...
    layer1 = tf.layers.dropout(layer1, keep_prob=dropout_keep_prob, is_training=is_training)
    ...
    layer2 = tf.layers.dropout(layer2, keep_prob=dropout_keep_prob, is_training=is_training)
    ...
    layer3 = tf.layers.dropout(layer3, keep_prob=dropout_keep_prob, is_training=is_training)
    ...
    layer4 = tf.layers.dropout(layer4, keep_prob=dropout_keep_prob, is_training=is_training)
    ...
    layer5 = tf.layers.dropout(layer5, keep_prob=dropout_keep_prob, is_training=is_training)
    ...
    layer6 = tf.layers.dropout(layer6, keep_prob=dropout_keep_prob, is_training=is_training)
    ...
    lienar1 = leaky_relu(linear1, 0.01)
